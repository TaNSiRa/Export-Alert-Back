...
function generateEmailHTML(statusMap) {
    const statusConfig = [
        ...
        { name: 'Send shipping document to customer', icon: '📤', color: '#673AB7' },
        { name: 'Issue invoice for accounting', icon: '🧾', color: '#4CAF50' },
        { name: 'Receive shipping&forwarder billing', icon: '💰', color: '#8BC34A' }
    ];
    ...
    statusConfig.forEach(config => {
        ...
        if (statusData.length > 0) {
            sectionsHTML += generateStatusSection(config.name, config.icon, config.color, statusData);
        }
    });
...
function generateSummaryStats(statusMap) {
    const totalShipments = Object.values(statusMap).reduce((sum, items) => sum + items.length, 0);
...
function generateStatusSection(statusName, icon, color, data) {
    ...
    <div class="shipment-card">
      <div class="card-header" style="background-color: ${color}15;">
        <div class="shipment-badge" style="background: linear-gradient(135deg, ${color}, ${adjustColor(color, -20)});">
          Shipment ${index + 1}
        </div>
        <div class="customer-name" style="color: ${color};">
          ${item.custshort || '-'}
        </div>
      </div>
      ...
      <div class="card-body">
        ...
        <div class="detail-row" style="border-left-color: ${color};">
          ...
          <div class="detail-content">
            ...
            <div class="detail-value">${item.etd || '-'}</div>
          </div>
        </div>
  ...
  `).join('');
    ...
    <div class="status-section">
      <div class="status-header" style="background: linear-gradient(135deg, ${color}, ${adjustColor(color, -20)});">
...
async function sendDailyReport(recipients) {
    try {
      ...
      ORDER BY user_input_date DESC;`;
        ...
        //     to: recipients.join(', '),
        //     subject: `📊 Daily Shipment Report - ${new Date().toLocaleDateString('th-TH')}`,
        //     html: htmlContent
        // };
...
function scheduleDailyEmail() {
    ...
    cron.schedule('30 8 * * *', async () => {
        console.log('⏰ Running scheduled daily report at', new Date().toLocaleString('th-TH'));
